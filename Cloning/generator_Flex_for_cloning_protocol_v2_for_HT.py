...
This program will guide you through an OT-2 cloning protocol design.
''')
    messagebox.showinfo("Select the fixed toolkit map", '''In the upcoming file browser, open the "Cloning" subfolder of Auto-GG and select "fixed_toolkit_map.csv" (file should have this exact name)''')
...
def ask_customised_dna_plate_map_filename():
    window = tkinter.Tk()
    window.withdraw()
    messagebox.showinfo("Choose the custom parts map", '''You will now choose "custom_parts_map.csv" (file should have this exact name)''')
...
def create_protocol(dna_plate_map_dict, combinations_to_make, protocol_template_path, output_folder_path):
	...
	with open(output_folder_path + '/' + 'protocol_for_cloning_YTK_' + str(today) + '.py', "w+") as protocol_file:
